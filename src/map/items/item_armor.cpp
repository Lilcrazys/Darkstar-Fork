...
{
	return dsp_cap(m_reqLvl+getModifier(Mod::REQUIRED_LV), 0, 255);
}

uint8 CItemArmor::getILvl()
{   //printf("(m_equipSlotID %u) getILvl: %u \n", m_equipSlotID, (getModifier(Mod::ITEM_LEVEL) > 0 ? dsp_cap(getModifier(Mod::EFFECTIVE_ILV)+99, 0, 255) : m_iLvl));
    return (getModifier(Mod::ITEM_LEVEL) > 0 ? dsp_cap(getModifier(Mod::ITEM_LEVEL)+99, 0, 255) : m_iLvl);
}
