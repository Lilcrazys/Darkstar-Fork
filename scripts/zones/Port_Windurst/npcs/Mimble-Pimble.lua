...
function onTrigger(player,npc)
    ...
    elseif (player:getMainLvl() < 90) then
        ...
        player:startEvent(895, A, phantomGems, MERITS, finished, honors, unknown2, unknown3, unknown4);
    end
end;
...
function onEventUpdate(player,csid,option)
    ...
    if (csid == 895 and (option == 1 or option == 3 or option == 259)) then
        local MERITS = player:getMeritCount();
        local unknown1 = 0; -- Dummied until understood
        local phantomGems = 0x000FFFFF;
        local finished = 0xFFFFFFFF;
...
--[[
    if (csid == 895) then
        ...
        elseif (option ~= 1073741824) then
            print("Invalid onEventFinish result in Mimble-Pimble.lua (Northern San d'Oria) csid: "..csid.."option: "..option);
        end

        if (keyItem ~= nil and cost ~= nil) then
            if (player:hasKeyItem(keyItem)) then
                player:SpoofChatPlayer("Mimble-Pimble : You already posses that Phantom Gem. ", MESSAGE_SYS_SAY, nil);
            else
                ...
                if (meritCount >= cost) then
                    ...
                    player:SpoofChatPlayer("Mimble-Pimble : You haven't enough. I can do nothing with that amount. ", MESSAGE_SYS_SAY, nil);
                end
            end
        end
    end]]
