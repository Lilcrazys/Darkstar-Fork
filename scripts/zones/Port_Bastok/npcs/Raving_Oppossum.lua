    ...
    if (csid == 429) then
        ...
        if (keyItem ~= nil and cost ~= nil) then
            if (player:hasKeyItem(keyItem)) then
                player:PrintToPlayer("Raving Opossum : You already posses that Phantom Gem. ", chatType.NS_SAY);
            else
                ...
                if (meritCount >= cost) then
                    ...
                    player:PrintToPlayer("Raving Opossum : You haven't enough. I can do nothing with that amount. ", chatType.NS_SAY);
                end
            end
        end
    end]]
