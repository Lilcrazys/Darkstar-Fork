    ...
    if (csid == 429) then
        ...
        if (keyItem ~= nil and cost ~= nil) then
            if (player:hasKeyItem(keyItem)) then
                player:SpoofMsg("Raving Opossum : You already posses that Phantom Gem. ", nil, MESSAGE_SYS_SAY, nil);
            else
                ...
                if (meritCount >= cost) then
                    ...
                    player:SpoofMsg("Raving Opossum : You haven't enough. I can do nothing with that amount. ", nil, MESSAGE_SYS_SAY, nil);
                end
            end
        end
    end]]
