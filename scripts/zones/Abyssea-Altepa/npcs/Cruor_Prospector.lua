...
function onEventFinish(player,csid,option)
    ...
    -- Keyitems
    elseif (option == 65539) then -- MAP_OF_ABYSSEA_ALTEPA
        if (player:hasKeyItem(MAP_OF_ABYSSEA_ALTEPA) == false) then
            player:messageSpecial(KEYITEM_OBTAINED,MAP_OF_ABYSSEA_ALTEPA);
            player:addKeyItem(MAP_OF_ABYSSEA_ALTEPA);
