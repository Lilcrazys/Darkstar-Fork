...
function onTrigger(player,npc)
    local STRATUM = player:hasKeyItem(CRIMSON_STRATUM_ABYSSITE_III);

    if (STRATUM == true) and npc:getXPos(78) and npc:getYPos(0) and npc:getZPos(118)  then
...
function onEventFinish(player,csid,option)
    ...
    if (cs == 6000 and option == 1) then
        if (player:hasKeyItem(CRIMSON_STRATUM_ABYSSITE_III)) then
            player:delCurrency("voidstones", 1);
            SpawnMob(17113826, 300):updateClaim(player);
        end
    end
end;