...
function onMobDeath(mob, killer, ally)
    ...
    if (ally:hasKeyItem(ASHEN_STRATUM_ABYSSITE_III)) then -- Aello Kill
        if (ally:getMaskBit(ally:getVar("ZILART_VW"), 0) == false) then
            ally:setMaskBit(ally:getVar("ZILART_VW"),"ZILART_VW",0,true);
        end
