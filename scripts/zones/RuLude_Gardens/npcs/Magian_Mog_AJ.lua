...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(ADOULIN,CHILDREN_OF_THE_RUNE) == QUEST_ACCEPTED) then
        ...
        if (trade:hasItemQty(1408,1) and trade:hasItemQty(16606,1) and itemCount == 2) then
            player:SpoofMsg("Now we bind your soul to this piece of ju..I mean fine crafted restoration! ", npc, MESSAGE_SAY, nil);
            ...
            player:SpoofMsg("..All done, kupo! You don't have the full strength of a Rune Fencer yet, but its a start! ", npc, MESSAGE_SAY, nil);
        else
            player:SpoofMsg("Sorry but I can't do anything with this, kupo.. ", npc, MESSAGE_SAY, nil);
        end
    else
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(ADOULIN,CHILDREN_OF_THE_RUNE) == QUEST_AVAILABLE) then
        player:SpoofMsg("I've been temporarily authorized to empower adventurers with RUNIC MAGIC kupo! ", npc, MESSAGE_SAY, nil);
        player:SpoofMsg("First we need a Rusty Greatsword and a Bottle of Illuminink. The blade has to be old for the magic to work.", npc, MESSAGE_SAY, nil);
        player:SpoofMsg("But don't worry, it won't be rusty when we're done. ", npc, MESSAGE_SAY, nil);
    elseif (player:getQuestStatus(ADOULIN,CHILDREN_OF_THE_RUNE) == QUEST_ACCEPTED) then
        player:SpoofMsg("we need a Rusty Greatsword and a Bottle of Illuminink, kupo. ", npc, MESSAGE_SAY, nil);
    else
    ]]-- End of RUN quest stuff
    -------------------------
        player:SpoofMsg("If neither TJ or CJ can help you, try the guys down in Port Jeuno. ", npc, MESSAGE_SAY, nil);
