...
function onTrigger(player,npc)
    ...
    if (player:getMainLvl() < 75) then
        player:PrintToPlayer("You must be at least level 75 to participate in Voidwatch. ", chatType.SYSTEM_3, npc:getName());
    else
        ...
        if (CRIMSON == false and INDIGO == false and JADE == false and STATUS == 0) then
            player:PrintToPlayer("Voidwatch Officer: I am a Voidwatch officer dispatched here to recruit soldiers to repel the Voidwalker threat. ", chatType.NS_SAY, npc:getName());
            player:PrintToPlayer("Voidwatch Officer: I beseech you, friend. Lend your aid to our cause! ", chatType.NS_SAY, npc:getName());
            player:setVar("VW_STATUS", 1);
        elseif (INDIGO == false) then
            player:PrintToPlayer("DEBUG: Initial Abyssite check started.");
            if (player:hasKeyItem(ADVENTURERS_CERTIFICATE) == false) then
                player:PrintToPlayer("Voidwatch Officer: I'm sorry, but only certified adventurers may participate in Voidwatch Ops. ", chatType.NS_SAY, npc:getName());
                player:PrintToPlayer("Voidwatch Officer: Serve your country well, and your rank is sure to improve. ", chatType.NS_SAY, npc:getName());
            elseif (player:getCampaignAllegiance() == 0) then
                player:PrintToPlayer("Voidwatch Officer: Hmm? I can't seem to find any military record.. ", chatType.NS_SAY, npc:getName());
                player:PrintToPlayer("Voidwatch Officer: I'm sorry, but only those with military affiliation may participate in Voidwatch Ops. ", chatType.NS_SAY, npc:getName());
            else
                player:PrintToPlayer("Voidwatch Officer: Stratum abyssite for each region is needed to battle a Voidwalker at the many planar rifts that have manifested across the land. ", chatType.NS_SAY, npc:getName());
                player:PrintToPlayer("Voidwatch Officer: You will also need a Voidstone to see the beast. Their powers can only be tapped when used in conjunction with a Stratum abyssite. ", chatType.NS_SAY, npc:getName());
        ...
        elseif
            ...
            if (player:getVar("VOIDSTONE_TIMER") < os.time()) then
                ...
                player:PrintToPlayer("Voidwatch Officer: While these stones are capable of rendering Voidwalkers visible to their bearers.. ", chatType.NS_SAY, npc:getName());
                player:PrintToPlayer("Voidwatch Officer: ..Their powers can only be tapped when used in conjunction with a Stratum Abyssite. ", chatType.NS_SAY, npc:getName());
                player:addCurrency("voidstones", 1);
                player:addKeyItem(VOIDSTONE1); -- so morons stop showing us their KI screenshots..
                player:setVar("VOIDSTONE_TIMER", os.time()+72000);
                -- 20 hours till next voidstone. Slightly less than 1 day,
                -- so that player can do VW once a day at aprox same time of day.
                player:messageSpecial(KEYITEM_OBTAINED, VOIDSTONE1);
                player:PrintToPlayer(string.format("You now posses %d Voidstones in total. ", player:getCurrency("voidstones")), chatType.SYSTEM_3, npc:getName());
            else
