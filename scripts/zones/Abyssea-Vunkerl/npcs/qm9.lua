...
function onTrade(player,npc,trade)
    ...
    elseif (GetMobAction(17666487) == ACTION_NONE) then -- mob not already spawned from this
        if (trade:hasItemQty(3098,1) and trade:getItemCount() == 2) then -- Player has all the required items.
            SpawnMob(17666487, 300):updateEnmity(player); -- Spawn NM, Despawn after inactive for 5 minutes (pt has to reclaim within 5 of a wipe)
