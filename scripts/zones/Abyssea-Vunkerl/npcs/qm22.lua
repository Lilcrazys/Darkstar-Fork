...
function onTrigger(player,npc)
    if (GetMobAction(17666501) == ACTION_NONE) then -- NM not already spawned from this
        ...
        and player:hasKeyItem(CHIPPED_IMPS_OLIFANT) then -- I broke it into 3 lines at the 'and' because it was so long.
            player:startEvent(1023, DECAYED_DVERGR_TOOTH, PULSATING_SOULFLAYER_BEARD, CHIPPED_IMPS_OLIFANT); -- Ask if player wants to use KIs
        else
            player:startEvent(1024, DECAYED_DVERGR_TOOTH, PULSATING_SOULFLAYER_BEARD, CHIPPED_IMPS_OLIFANT); -- Do not ask, because player is missing at least 1.
        end
    end
end;
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 1023 and option == 1) then
        SpawnMob(17666501, 300):updateEnmity(player); -- Spawn NM, Despawn after inactive for 5 minutes (pt has to reclaim within 5 of a wipe)
